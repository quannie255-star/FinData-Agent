...
### v3.0.2 · 第二轮拷问：值错位证据 + 前提敏感度实验 + 删掉一个假探针（2026-09-20）

第二份拷问清单（更细，判据写明「你到底是写了个规则脚本，还是真的在做训练
数据质检系统」）逐条自查。三个动作 + 一次自我否定。

**1. 查出比「漏标 default」更硬的一层证据：`default` 贴错了位置。**
`calculate_electric_field`（`row 65`）的原始 schema：
...
8.854e-12 是**真空介电常数**，不可能是库仑计的电荷量（还是 `int` 型）的
默认值。**不是"忘了写"，是"值写在了错的参数上"**——疏忽解释不通。
全量这类只有 **2 个参数位**（`permitivity` 258 次 / `confidence` 83 次），
其余 19 项是纯措辞。**处置分型：19 项"补上 default" vs 2 项"把值移过去"**
（统一成一句「补 default」，数据源方会给两边同时标 8.854e-12，改得更错）。

**2. 删掉一个假探针（本轮最重要的一次自我否定）。**
新写了「类型与默认值是否自相矛盾」的纯机械规则——不依赖任何 description
措辞，全量 **2162 命中**。抽三条读原文发现：**该数据源 89.1% 的 `default`
本就是字符串**（`type: int` + `default: "10"`），是表示层约定不是矛盾。
**探针当场删除**，并把这次否定写进报告第八节。

**3. 新增前提敏感度实验**（`scripts/sensitivity_premise.py`，
归档 `examples/premise-sensitivity.txt`）。我的核心判据「没标 `default`
即必填」**是我自己设的口径**（该数据集 `required` 字段为 0），
于是把它整个翻转再跑一遍：
...
结论：**丢弃数不因前提翻转而变**——押错前提不会导致误杀，
那批样本只是从「待修 schema」搬进「需人工」。
（该实验第一版 patch 错了函数，两遍跑出完全相同的数字，
**"稳健"差点是个假结论**；现已加断言先证明 patch 生效。）

**4. 口径统一 + 措辞修正。**
主闭环由 `--limit 20000` 抽样改为**默认全量 6 万条**（16 秒）——
抽样比例与全量不是一回事，而所有手工核证都在全量上做。
全量口径：**60000 条 / 100011 次调用 / 命中 1351（2.25%）/
A 65 / B 1286 / 21 个工具 21 个参数 / 364 例测试**。
另修两处我自己犯的措辞错：报告第二节原把 `ok`（=未命中）放在
「**根因**分布」标题下（等于替数据判过了）；第八节把「数据源没声明必填」
写成「因此模型省略是合理的」——**这是加进去的推断，不是那个事实的推论**。

**5. 交付 `docs/interview-qa-round2.md`**：Q1–Q37 + 五个红旗逐条 + 开场三句话
+ **本轮自我暴露的 6 处对我不利的事实** + 不自证清单 12 条。

### v3.0.1 · 面试拷问自查：误杀修正 + 措辞纪律 + 可复现三元组（2026-09-18）

起因是拿一份 37 条的面试官拷问清单逐条自查。**不辩解，先取证**：两条被当场
证实成立，直接改代码。

- **误杀 349 条（最严重）**：`CAT_SCHEMA_CONTRADICTION` 原判 `✗ 丢弃`。
  取证：`Salesforce/xlam-function-calling-60k` 的 **58105 个 tool 里带
  `required` 字段的是 0 个**、125120 个参数条目里 92884 个（74%）带
  `default`——数据源从未声明过"必填"，「没 default 即必填」是我自造的口径。
  处置新增第五档 **`🔧 待修 schema`**：样本留着，去改数据源。
  配套 `ToolDefectTally` 聚合出 `tool_schema_defects.json`
  （**18 个工具 / 18 个参数**）——同一份事实，样本级表述是"349 条要人工看"
  （等于没有队列），工具级表述是"18 个工具的 schema 要改"（可执行）。
  实测分布：`✓ 正样本 19646 / 🔧 待修 schema 349 / ✗ 丢弃 5`，
  **修正后一条样本都不用丢**
- **措辞纪律**：报告第一节由「发现率」改「命中率」，显式加注
  **「命中数不等于错误数」**；「干净」改「未命中」并加注
  **「未命中 ≠ 确认正确」**（无执行反馈时只能这么说）
- **可复现三元组**：`manifest.reproducibility` =
  `code_commit` + `rules_version`（`triage.RULES_VERSION`，**改判定逻辑必须
  动这个号**）+ `data`（sha256 前 16 位 + 字节数）。只记 commit 不够：
  规则常量可能改了没提交，落盘样本照样与代码对不上号
- **文档一致性门禁** `tests/test_docs_consistency.py`（3 例）
  - 第一道：全仓库对"当前测试数"只能有一个值。上线即抓出 `PITCH.md:87`
    漏网的 351
  - 第二道：**文档写的数字必须等于实际收集到的用例数**（跑
    `--collect-only`，2 秒，不执行用例体所以不递归）。只查"文档之间不打架"
    不够——它们可以一起写错。本轮 354 → 357
  - 顺带查出 `354` 在文档里**同时表示"测试数"和"命中数"**两个语义，
    且两处产物清单是改判前的旧值（`discard.jsonl 354`）→ 已全部回填实值
- 测试 356 → 357（新增门禁 1 例）
