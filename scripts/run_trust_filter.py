...
def run(limit: int, path: str | None, archive: Path | None, out_dir: Path | None) -> int:
    ...
    a("")
    a("二、判定分布")
    a("  注：`ok` 那一行是**未命中**——它只说明「我能查的那几项没查出问题」，")
    a("  **不是**「确认这批数据没问题」。把未命中写进叫「根因」的表里，")
    a("  等于我替这批数据判过了。")
    ...
    a("  · 该数据集**没有 `required` 字段**：本批扫过的 "
      f"{n_tool_defs} 份工具定义里，带 `required` 的是 {n_with_required} 份。")
    a("    这一条**只能推出**：「哪些参数必填」不是数据源说的。")
    a("    而「模型省略一个描述里写着有默认值的参数是合理行为」——**这是我加上去的")
    a("    推断，不是那个事实的推论**。（原文早先写成「因此」，是个逻辑跳跃，已改。）")
    a("    我保留这个推断，因为它让处置落在**可逆**的一侧：改 schema 而不是丢样本。")
    a("    押错前提会怎样，见 `examples/premise-sensitivity.txt`——把前提整个翻转")
    a("    后丢弃数仍是 0：那批样本从「待修 schema」搬进「需人工」，一条没少。")
