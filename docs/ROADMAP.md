...
> 正则当成"全仓库测试数"的另一个声明——那一条要求全仓库只有一个数（当前 532）。
> 模块级计数用"个用例"，别改回去。
...
**R5.2 提议与验证**（**本阶段的核心**）
- [x] 至少产出 **1 个具体的工具返回裁剪建议**，并在沙箱里重放验证
      → `scripts/build_compact_profile.py`（从 trace 学上限）+ `compaction.py` 四档策略，
      32 任务 × 4 臂全部重放（`--tag active7b-{fx,ch,ci,cp}`）
- [x] **三组数字齐全**：token 减少 %（**实测** provider usage）/ 折算成本减少 %
      （**换算**，单价公开且标注）/ 质量变化（四档 `hit` / `process_ok` / `verified_hit` /
      **`exact_hit` 为准** + 配对检验）。**不许把换算值写成实测值**
- [x] **对照两组**（**2026-09-20 改设计，理由见 `docs/competitive-landscape.md` §10**）
      - 原设计「字段裁剪 vs RouteLLM 式模型路由」**作废**：两者不同层；且 Jev 已是
        路由这一路的事实标准（LangChain `TypeSafeClassifier` / Vercel AI SDK 7）
        → **不自己重写**
      - 新设计 = **同层对手**：「**合理性判断裁剪**」（`head`：只看"有多长"，
        设计者常数 500 字符；`hint`：整段换一行提示 = 该协议**仿真**）
        vs「**trace 反推裁剪**」（`cap`：上限学自另一批 trace 的 p90）
      - **已按纪律标注** `hint` 是**协议仿真，不是 Jev 本体**（脚本输出与测试各写一遍）
- [x] **证据强度分层写进产物**：profile 里给**原始计数** `n` / `n_ref`（不只给比例），
      `keep_rule` 写明「`ref_rate == 0` = 本批观测到 0 次引用，**不是证明无用**」，
      处置**只做整类丢不逐条判死**，且**丢了仍然可逆**（模型还能再调这个工具，
      存储里没有任何东西被删）——这是 §2.1 那条判据在 R5.2 上的落法

> **R5.2 已跑完的读数（7B / 32 任务 / 4 工具 / 同一语料快照 + 同一份代码）**：
> 裁 **88.7%** 字符 ⇒ prompt token **−25.8%**、`exact_hit` **7/32→7/32**（p=1.0，
> 且 k=6 越过分辨力下限 ⇒ **这是「没测到伤害」，不是「测不出」**）；
> 裁 **95.9%** ⇒ **−15.1%**、`exact_hit` **7/32→1/32**（**p=0.0078 显著变差**）。
> ⇒ 「**少给内容**」和「**不给内容**」不是程度差异，是两种东西。
> 正文与复现命令见 `docs/r5.0-acceptance.md` §3.8；结果表由
> `scripts/compare_compaction_arms.py` 产出（会核对各臂语料指纹与**代码指纹**，
> 支持 `--require-same-corpus` / `--require-same-code`）。

**R5.3 材料**
- [ ] 一个真实 agent 的端到端案例，含改前/改后 trace 对比
- [x] README / PITCH 主线切到 v4.0；v3.0 降为领域包 2
      （README 标题与首屏改为上下文策展、PITCH 新增「零·新」简历段与「三·新」电梯演讲）
