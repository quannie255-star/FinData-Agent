...
- [ ] **对照两组**：① 只做"上下文字段裁剪" ② 只做 RouteLLM 式"模型路由"
      → 说明两者是叠加还是冲突（不做单方面宣称）
      > **②那一臂的预备读数**（R5.0 §2.2）：把模型从 7B 降到 3B，在本地这批
      > 工具调用任务上是**负收益**（命中 0/6 vs 4/6，且 token 更高）——
      > **但 n=6、p=0.125 不显著**，所以正式那一臂要先把 3B 跑满 32 任务
      > （`--tag all3b`），否则不许引用这个读数。
      > 另外这条**不能直接推广**到 RouteLLM 的场景（它按任务难度路由，
      > 不是全线降级），但足以说明：**"路由"的收益不是先验正的，必须实测**。
