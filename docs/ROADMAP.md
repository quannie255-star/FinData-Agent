...
探针先做 5 类（形态信号，不含判断）：
`degrade`（降级）、`abort`（中断）、`retry_storm`（重试风暴）、
`hallucinated_slot`（槽位无出处，如把"9 月"编成 9 月 4 日）、
`silent_mismatch`（跑完了但口径与事实不符）。

- 验收门禁（**先写后做**）：
  - [ ] 48 条真实轨迹跑质检，逐条判定可复核（归档 `examples/`）
  - [ ] loud / silent 两档严重度判定单测钉死（判反=结论相反，见 `tests/test_agentops.py`）
  - [ ] **归因准确率 ≥ 90%**：用 R4.2 的已知根因合成集测——
        这是本项目第一个**有 ground truth 的归因评测**，以前都是人工复盘

### R4.2 合成管线：造「根因已知」的失败样本

为什么必须自己造：真实轨迹的根因是人肉复盘出来的，无法规模化，
也无法算准确率。合成样本的**根因是写进去的**，所以能反过来测归因器。

- 用 Multi-Agent 生成任务场景与长程轨迹（本身也是对 JD9 第 1 条的实践）
- 每类失败注入已知根因：参数写错 / 工具不存在 / 沙箱超时 / 重试死循环 /
  幻觉槽位 / 蒙对
- 验收：每类 ≥ 20 条，根因标签入库；**合成器自己也要能被证伪**
  （注入的失败必须真的在轨迹里留下可观测信号，否则是无效合成）

### R4.3 闭环工程化：合成 → 运行 → 评测 → 过滤

JD9 第 3 条原话：「将『数据合成 → 沙箱运行 → 自动评测 → 质量过滤』的
闭环流程进行脚本集成与 Pipeline 工具化」。

- 一条命令跑完四环，产物是可直接喂 SFT/RL 的正负样本集 + 质检报告
- 验收：端到端可复现；过滤前后样本量如实入账（不藏丢弃率——
  **丢弃率高不是坏事，说明真的在过滤**）
