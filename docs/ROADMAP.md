...
> **0/6**，且 prompt_tokens **更高**（30,548 > 28,516）——**换小模型既贵又不准**。
> ⚠️ **但 n=6、McNemar p=0.125，不显著**，只能当方向性观察；
> 处置：把 3B 也跑满 32 个任务再下结论（见 R5.0 验收记录 §2.2.1）。
...
- [ ] **对照两组**（**2026-09-20 改设计，理由见 `docs/competitive-landscape.md` §10**）
      ...
      > R5.0 的 3B vs 7B 预备读数是**负收益**（命中 0/6 vs 4/6、token 更高），
      > **但 n=6、McNemar p=0.125 不显著** → 要先跑 `--tag all3b`（32 任务）
      > 才允许引用。见 `docs/r5.0-acceptance.md` §2.2 / §2.2.1。
