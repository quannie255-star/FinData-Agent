...
>    而不是加同构样本。
> 2. **2×2 交互（工具长度 × 模型）**：token 侧**无显著交互**（逐任务交互项
>    18 正 / 14 负，**p = 0.5966**，**中位交互仅 +27 token**）⇒
>    「小模型受工具膨胀的 token 代价更大」**没有证据**。机制上也说得通：
>    定义是每轮固定重发的常量，两臂差主要由「定义长度差 × 轮数」决定。
> 3. **但行为侧差异看得见**：3B 的工具调用数 27→4 工具时 **44 → 62（+41%）**，
>    零工具调用任务 **7 → 3**；7B 是 54 → 55。⇒ 假设是「清单过长让小模型更不肯查」，
>    **只有两个描述性数字、未做检验，只能当机制假设**。
>    另有对我不利的一条：3B 有 **1 个任务长清单命中、短清单丢**（7B 是 0）
>    ⇒ **"裁剪至少不降质量"在 3B 上没有证据支持**。
>
> 详见 `docs/r5.0-acceptance.md` §2.2 / §2.2.1。
...
- [ ] **对照两组**（**2026-09-20 改设计，理由见 `docs/competitive-landscape.md` §10**）
      ...
      > McNemar p=0.2891 不显著**；且 2×2 的 token 侧交互 **p=0.5966 不显著**。
      > ⇒ 现阶段只能说"**换成 3B 没有得到可辨识的收益或损失**"，
      > **不能**说"路由到小模型更划算/更差"。见 `docs/r5.0-acceptance.md` §2.2.1。
