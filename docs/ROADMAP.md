...
> R5.0 的 3B vs 7B 对照（同 harness、同任务、同工具清单）显示：3B 关键词命中
> **0/6**，且 prompt_tokens **更高**（30,548 > 28,516）——**换小模型既贵又不准**。
> 这条同时也是 R5.2「模型路由」那一臂的**预备读数**，见下。
...
- [ ] **对照两组**：① 只做"上下文字段裁剪" ② 只做 RouteLLM 式"模型路由"
      → 说明两者是叠加还是冲突（不做单方面宣称）
      > **②那一臂已经有预备读数**（R5.0 §2.2）：把模型从 7B 路由到 3B，
      > 在本地这批工具调用任务上是**负收益**（0/6 命中 vs 4/6，且 token 更高）。
      > 这条**不能直接推广**到 RouteLLM 的场景（它按任务难度路由，不是全线降级），
      > 但足以说明一件事：**"路由"这个动作的收益不是先验正的，必须实测**。
      > 正式那一臂要按难度分层路由，不能整臂换模型。
